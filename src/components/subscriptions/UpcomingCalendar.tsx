...
export const UpcomingCalendar: React.FC = () => {
  ...
  // Compute monthly amortized cost
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          <h3 className="text-lg font-bold text-slate-900 dark:text-white">Recurring Bills & Subscriptions</h3>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Never get surprised by automated recurring charges or forgotten trials
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-4 rounded-2xl bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800 shadow-sm">
          ...
          <div className="text-xs text-slate-400 mt-1">
            Amortized across all active services
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800 shadow-sm">
          ...
          <div className="text-xs text-slate-400 mt-1">
            1-year projected recurring expenses
          </div>
        </div>
      ...
      {sortedSubs.length === 0 ? (
        <div className="py-16 text-center rounded-2xl bg-white/50 dark:bg-slate-900/50 border border-slate-200/60 dark:border-slate-800 p-8">
          ...
          <h4 className="text-base font-bold text-slate-900 dark:text-white">No Subscriptions Added</h4>
          <p className="text-xs text-slate-400 max-w-sm mx-auto mt-1 mb-4">
            Add recurring memberships like Netflix, Spotify, gym, or apartment rent to track renewal dates.
          </p>
          <button
            onClick={() => handleOpenAdd()}
